...
function parseBookingItem(item) {
  ...
  // Trying known Podio shapes defensively — see note at top of lib/podio.js.
  const startRaw =
    dateVal.start || dateVal.start_utc || (dateVal.start_date ? `${dateVal.start_date} ${dateVal.start_time || '00:00:00'}` : null);
  const endRaw =
    dateVal.end || dateVal.end_utc || (dateVal.end_date ? `${dateVal.end_date} ${dateVal.end_time || '00:00:00'}` : null);
